...
# Atuin only records preexec's $1. nsh clears the buffer before accept-line, so
# `? …` never reaches Atuin. Start/end an explicit history row for NL queries.
if typeset -f __nsh_handle_nl_query_line >/dev/null 2>&1; then
  ...
  # Quoted subscripts on functions[...] break the copy in zsh (orig never defined).
  ...
  __nsh_handle_nl_query_line() {
    ...
    '?? '* | '?! '* | '? '*)
      if command -v atuin >/dev/null; then
        ...
        __NSH_ATUIN_NL_ID="$(ATUIN_LOG=error atuin history start -- "$BUFFER" 2>/dev/null)"
      fi
      ;;
    esac
  ...
  unset _nsh_fn_orig _nsh_fn_src
  __nsh_run_deferred() {
    local _atuin_nl_id="${__NSH_ATUIN_NL_ID:-}" _atuin_t0="${EPOCHREALTIME-}" _dur
    ...
    if [[ -n "$_atuin_nl_id" ]]; then
      ...
      if [[ -n "$_atuin_t0" ]]; then
        ...
        [[ -n "$_atuin_t1" ]] && printf -v _dur %.0f $(((_atuin_t1 - _atuin_t0) * 1000000000))
      fi
      (ATUIN_LOG=error atuin history end --exit "$_atuin_qexit" ${_dur:+--duration=$_dur} -- "$_atuin_nl_id" &) >/dev/null 2>&1
    fi
    return "$_atuin_qexit"
  }
fi
