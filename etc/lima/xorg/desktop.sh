...
apt-get install -y --no-install-recommends "${APTOPT[@]}" xorg xserver-xorg-video-dummy python3-pip
...
CB=$!
# no display manager: video.display none = no GPU -> no seat graphics device,
# lightdm never spawns X. systemd units run Xorg dummy + xfce directly.
