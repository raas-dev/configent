...
Installer requires only `curl` available:
    ...
    curl -fsSL https://raw.githubusercontent.com/raas-dev/configent/1.185.1/install.sh | sh
...
The containers run on dockerd and the image is built using `etc/nix/Dockerfile`.
