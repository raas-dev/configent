...
- Uses AI coding agents to automate web browsing and computer use workflows.
...
- One character command `alias`es. The less typing, the faster it happens.
...
- Ask `ai <question>` or agent (`_`) to use web browser and other desktop apps.
...
Installer requires only `curl` available:
    ...
    curl -fsSL https://raw.githubusercontent.com/raas-dev/configent/1.221.14/install.sh | sh
...
The containers run on Podman and the image is built using `etc/nix/Dockerfile`.
